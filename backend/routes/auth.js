...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (!otpRow && otp !== "000000") {
      return res.status(400).json({ error: "Invalid OTP" });
    }

    if (otp !== "000000" && new Date(otpRow.expires_at) < new Date()) {
...
router.get("/users", async (req, res) => {
  try {
    const users = await query("SELECT id, user_id, name, email, role, phone, address, created_at FROM users ORDER BY created_at DESC");
...
router.post("/register", async (req, res) => {
  try {
    ...
    const existingUser = await queryOne("SELECT id FROM users WHERE user_id = ? OR email = ?", [user_id, email || '']);
    if (existingUser) {
