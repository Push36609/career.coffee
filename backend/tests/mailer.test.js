...
mock.method(nodemailer, 'createTransport', config => {
  ...
  return {
    ...
    sendMail: async message => {
      ...
      return { messageId: 'test', accepted: [message.to].flat(), rejected: [] };
    },
  };
});
...
test('admin alert uses configured addresses and escapes appointment HTML', async () => {
  ...
  assert.equal(email.to, 'ppal36609@gmail.com');
...
test('production booking and confirmation alerts go to both admins and ignore the old recipient setting', async () => {
  process.env.NODE_ENV = 'production';
  try {
    for (const event of ['booked', 'confirmed']) {
      assert.equal(await mailer.sendAppointmentNotificationEmail({ id: 42, email: 'student@example.com' }, event), true);
      assert.deepEqual(sent.at(-1).to, ['ppal36609@gmail.com', 'blessednishant@gmail.com']);
    }
  } finally {
    process.env.NODE_ENV = 'test';
  }
...
test('an unset environment sends local alerts to the development recipient', async () => {
  delete process.env.NODE_ENV;
  try {
    assert.equal(await mailer.sendAppointmentNotificationEmail({ id: 42, email: 'student@example.com' }), true);
    assert.equal(sent.at(-1).to, 'ppal36609@gmail.com');
  } finally {
    process.env.NODE_ENV = 'test';
  }
});
