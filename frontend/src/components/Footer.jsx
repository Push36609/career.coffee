    ...
    <footer className="bg-secondary-900 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            ...
            <div className="flex gap-3">
              {[
                ...
                <a key={href} href={href} target="_blank" rel="noopener noreferrer"
                  className={`w-9 h-9 bg-white/10 rounded-lg flex items-center justify-center ${color} transition-all duration-300 hover:scale-110`}>
                  <Icon size={16} />
                </a>
              ))}
            </div>
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-display font-semibold text-primary-300 mb-4 text-lg">Quick Links</h4>
            <ul className="space-y-2.5">
              {[['Home', '/'], ['About Us', '/about'], ['Services', '/services'], ['Exam Alerts', '/exam-alerts'], ['Blogs', '/blogs'], ['Testimonials', '/testimonials'], ['Contact', '/contact']].map(([label, path]) => (
                <li key={path}>
                  <Link to={path} className="text-white/60 hover:text-primary-300 text-sm transition-colors flex items-center gap-2 group">
          ...
          {/* Services */}
          <div>
            <h4 className="font-display font-semibold text-primary-300 mb-4 text-lg">Services</h4>
            <ul className="space-y-2.5">
              {['Career Counselling', 'School Counselling', 'College Admissions', 'Study Abroad', 'Students & Parents', 'Compliance Guidance', 'Exam Preparation', 'Psychometric Tests'].map(s => (
                <li key={s}>
                  <Link to="/services" className="text-white/60 hover:text-primary-300 text-sm transition-colors flex items-center gap-2 group">
                    <span className="w-1 h-1 bg-primary-500 rounded-full group-hover:w-2 transition-all duration-300"></span>
                    {s}
                  </Link>
                </li>
          ...
          {/* Contact */}
          <div>
            <h4 className="font-display font-semibold text-primary-300 mb-4 text-lg">Contact Us</h4>
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                <MapPin size={18} className="text-primary-400 mt-0.5 shrink-0" />
                <p className="text-white/60 text-sm">Career Coffee, B-39, 1st Floor, Middle Circle, Connaught Place, New Delhi - 110001</p>
              </div>
              <div className="flex items-center gap-3">
                <Phone size={18} className="text-primary-400 shrink-0" />
                <a href="tel:+916394619776" className="text-white/60 hover:text-primary-300 text-sm transition-colors">+91 63946 19776</a>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-5 h-5 bg-green-500 rounded-lg flex items-center justify-center shrink-0">
                  <Phone size={12} className="text-white" />
                </div>
                <a href="https://wa.me/916394619776" target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-green-400 text-sm transition-colors">WhatsApp Us</a>
              </div>
              <div className="flex items-center gap-3">
                <Mail size={18} className="text-primary-400 shrink-0" />
                <a href="mailto:info@careercoffee.in" className="text-white/60 hover:text-primary-300 text-sm transition-colors">info@careercoffee.in</a>
              </div>
            </div>
            <div className="mt-5 p-4 bg-primary-500/10 border border-primary-500/20 rounded-xl">
              <p className="text-primary-300 text-xs font-semibold uppercase tracking-wider mb-1">Office Hours</p>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            {['Privacy Policy', 'Terms of Service', 'Sitemap'].map(t => (
              <a key={t} href="#" className="text-white/40 hover:text-primary-300 text-sm transition-colors">{t}</a>
            ))}
          </div>
        </div>
      </div>
